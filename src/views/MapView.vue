...
export default {
  ...
  data() {
    return {
      ...
      enableTooltip: true,
      zoom: 15,
      center: [-6.6134095520467415, 106.75084517623543],
      layerData: [],
      geojson: null,
      fillColor: "#0CF9E0",
      tileProviders: tileProviders,
      marker: latLng(-6.6134095520467415, 106.75084517623543),
      timeout: undefined,
    };
  },
  mounted() {
  ...
  methods: {
    async loadSomeGeoJson() {
      ...
      const response = await fetch("Desa-Sukamakmur.json");
  ...
  async created() {
    this.loading = true;
    const response = await fetch("Landuse-Sukamakmur.json");
    const data = await response.json();
    this.geojson = data;
    this.loading = false;
  },
